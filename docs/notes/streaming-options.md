...
| --- | --- | --- |
| Selective scale compression with raw packed reads | Storage; a little I/O; decode allocation | Implemented here, opt-in, synthetic native byte gates. Real conversion size and timing remain unmeasured. |
...
| Direct-I/O alignment or Darwin cache bypass for selective raw spans | Page-cache pressure and copies | Implemented in the follow-up: raw extents are padded to their logical alignment and their aligned interiors go through the plain shards' direct descriptor; ragged ends and scale frames stay buffered, and a refused direct read falls back. Unmeasured on a real model. |
