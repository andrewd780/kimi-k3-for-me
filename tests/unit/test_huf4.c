...
int main(void)
{
    ...
    for (size_t n = 1; n <= 1024; n++) {
        ...
        if (huf4_decode(&table, src, len, low, out, n) || memcmp(raw, out, n)) return 1;
        if (len[0]) {
            len[0]--;
            if (!huf4_decode(&table, src, len, low, out, n)) return 1;
        }
    }
    ...
    if (!huf4_decode(&table, src, len, low, out, 2)) return 1;
