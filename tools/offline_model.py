...
class Reader(io.RawIOBase):
    ...
    def __init__(self, path):
        ...
        try:
            ...
            if (magic != MAGIC or reserved or count != block_count(self.size, self.block)):
            ...
            table = self.file.read(count * ENTRY.size)
            if len(table) != count * ENTRY.size:
                raise ValueError("short block index")
            self.entries = list(ENTRY.iter_unpack(table))
            bound = self.codec.lib.ZSTD_compressBound(self.block + PREFIX.size)
            for offset, size, flags in self.entries:
                if (offset != cursor or not 9 <= size <= bound or flags not in (0, 1)
                        or size > physical - cursor):
    ...
    def _block(self, index):
        ...
        size = min(self.block, self.size - index * self.block)
